...
/// </blockquote>
/// <blockquote>
/// Warning: Make sure to call the <code>setupWith(configuration:delegate:completionHandler:)</code> method to set the SDK up before calling any other methods.
///
/// </blockquote>
...
/// warning:
/// Use this method only in the scenario of the driver logout.
...
/// Get info on the currently active drive. If the SDK is not setup or if
/// no drive is in progress, nil is returned.
///
/// returns:
/// The currently active drive information.
+ (FairmaticActiveDriveInfo * _Nullable)activeDriveInfo SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("This method is deprecated as of SDK version 3.0.0. Please use getActiveDriveInfo(completion:) instead.");
/// Get info on the currently active drive. If the SDK is not setup or if
/// no drive is in progress, <code>nil</code> is passed to the completion handler.
/// \param completion The completion called with the currently active drive information.
///
+ (void)getActiveDriveInfoWithCompletion:(void (^ _Nonnull)(FairmaticActiveDriveInfo * _Nullable))completion;
...
/// note:
/// This method is deprecated as of SDK version 3.0.0. Use <code>getSettings(completionHandler:)</code> instead. Using this method might result in undefined behavior.
///
/// returns:
/// A valid <code>Settings</code> object if the SDK is setup, otherwise nil.
+ (FairmaticSettings * _Nullable)getSettings SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("This method is deprecated as of SDK version 3.0.0. Please use getSettings(completionHandler:) instead.");
...
///
/// returns:
/// A boolean indicating whether the background task registration was successful or not. <code>true</code> if the launch task was registered, <code>false</code> if the identifier isn’t included in the <code>BGTaskSchedulerPermittedIdentifiers</code> in the  <code>Info.plist</code>.
/// When invoked on a simulator, this method <em>always</em> returns <code>false</code>
+ (BOOL)registerBackgroundTask SWIFT_WARN_UNUSED_RESULT;
@end
